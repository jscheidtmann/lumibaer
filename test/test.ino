...
void loop() {
  ...
  oneColorChase(strip.Color(255,255,255), 50);
  oneColorChase(strip.Color(255,  0,  0), 50);
  oneColorChase(strip.Color(  0,255,  0), 50);
  oneColorChase(strip.Color(  0,  0,255), 50);
}
...
void colorWipe(uint32_t color, int wait) {
  for(int i=0; i<strip.numPixels(); i++) { // For each pixel in strip...
    strip.setPixelColor(pinTranslate(i), color);         //  Set pixel's color (in RAM)
  ...
  for(int a=0; a<10; a++) {  // Repeat 10 times...
    for(int b=0; b<3; b++) { //  'b' counts from 0 to 2...
      ...
      for(int c=b; c<strip.numPixels(); c += 3) {
        strip.setPixelColor(pinTranslate(c), color); // Set pixel 'c' to value 'color'
      }
...
void rainbow(int wait) {
  ...
  for(long firstPixelHue = 0; firstPixelHue < 5*65536; firstPixelHue += 256) {
    for(int i=0; i<strip.numPixels(); i++) { // For each pixel in strip...
      ...
      strip.setPixelColor(pinTranslate(i), strip.gamma32(strip.ColorHSV(pixelHue)));
    }
    strip.show(); // Update strip with new contents
    onboardLedSwitch();
    delay(wait);  // Pause for a moment
...
void oneColorChase(uint32_t color, int wait) {
  for(int i=0; i < LED_COUNT; i++) {
    strip.clear();
    strip.setPixelColor(pinTranslate(i), color);
    strip.show();
    onboardLedSwitch();
    delay(wait);
  }
